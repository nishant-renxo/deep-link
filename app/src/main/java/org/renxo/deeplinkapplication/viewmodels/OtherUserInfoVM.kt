                ...
                if (result.result?.code == AppConstants.SuccessCodes.SUCCESS200) {
                    result.params?.let { params ->
                        ...
                        params[AppConstants.Params.contacts]?.let {
                            json.decodeFromString<DetailResponse>(
                                it
                            ).let { con ->
                                contact = con
                        ...
                        params[AppConstants.Params.template]?.let {
                            json.decodeFromString<TemplateData>(
                                it
                            ).let { con ->
                                svg = con.svg
                            }
    ...
    fun saveContact(context: Context) {
        viewModelScope.launch {
            ContactInfo(context).saveContact(
                name =
                    contact?.fields?.name.toString(),
//                phoneticName =
//                    contact?.display_name.toString(),
//                company =
//                    contact?.fields?.company.toString(),
//                jobTitle =
//                    contact?.fields?.job_title.toString(),
